...
int main() {
    ...
    cl_command_queue commandQueue = clCreateCommandQueue(ctx, deviceId, 0, &errCode);
    OCL_SAFE_CALL(errCode);
    ...
    cl_mem as_gpu = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(), &errCode);
    OCL_SAFE_CALL(errCode);
    cl_mem bs_gpu = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(), &errCode);
    OCL_SAFE_CALL(errCode);
    cl_mem cs_gpu = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, sizeof(float) * n, nullptr, &errCode);
    OCL_SAFE_CALL(errCode);
    ...
    const char* kernelName = "aplusb";
    cl_kernel kernel = clCreateKernel(prog, kernelName, &errCode);
    OCL_SAFE_CALL(errCode);
