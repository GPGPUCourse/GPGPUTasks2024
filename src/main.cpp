...
int main() {
    ...
    OCL_SAFE_CALL(clGetDeviceIDs(platformId, CL_DEVICE_TYPE_CPU, 1, &deviceId, nullptr));
    ...
    cl_mem as_gpu =
            clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(), &err);
    OCL_SAFE_CALL(err);
    cl_mem bs_gpu =
            clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(), &err);
    OCL_SAFE_CALL(err);
    ...
    {
        std::ifstream file(
                "/home/vlad/Sasha/4_Course/courses/computing_on_video_cards/GPGPUTasks2024/src/cl/aplusb.cl");
    ...
    err = clSetKernelArg(kernel, 0, sizeof(as_gpu), &as_gpu);
    if (err != CL_SUCCESS) {
        std::cerr << "Error setting kernel argument: " << err << std::endl;
        return 1;
    }

    err = clSetKernelArg(kernel, 1, sizeof(bs_gpu), &bs_gpu);
    if (err != CL_SUCCESS) {
        std::cerr << "Error setting kernel argument: " << err << std::endl;
        return 1;
    }

    err = clSetKernelArg(kernel, 2, sizeof(cs_gpu), &cs_gpu);
    if (err != CL_SUCCESS) {
        std::cerr << "Error setting kernel argument: " << err << std::endl;
        return 1;
    }
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            ...
            // clWaitForEvents...
            err = clEnqueueNDRangeKernel(queue, kernel, 1, nullptr, &global_work_size, &workGroupSize, 0, nullptr,
                                         &event);
            if (err != CL_SUCCESS) {
            ...
            if (err != CL_SUCCESS) {
                std::cerr << "Error waiting for events: " << err << std::endl;
                return 1;
            }
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        double vramBandwidth = static_cast<double>(3 * n * sizeof(cl_int)) / (1024.0 * 1024.0 * 1024.0 * t.lapAvg());
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            // clEnqueueReadBuffer...
            err = clEnqueueReadBuffer(queue, cs_gpu, CL_TRUE, 0, sizeof(cl_int) * n, cs.data(), 0, nullptr, nullptr);
            if (err != CL_SUCCESS) {
        ...
        double dataTransferRate = static_cast<double>(n * sizeof(cl_int)) / (1024.0 * 1024.0 * 1024.0 * t.lapAvg());
        std::cout << "VRAM -> RAM bandwidth: " << dataTransferRate << " GB/s" << std::endl;
    }
