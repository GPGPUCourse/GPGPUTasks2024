...
int main() {
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        // OCL_SAFE_CALL(clGetPlatformInfo(platform, 836, 0, nullptr, &platformNameSize));
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            cl_ulong deviceMem;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, deviceMemSize, &deviceMem, NULL));
            std::cout << "    Device memory: " << to_string(deviceMem / 1000000) + " Mb" << std::endl;

            size_t deviceAddresBitsSize = 0;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_ADDRESS_BITS, 0, nullptr, &deviceAddresBitsSize));
            cl_uint deviceAddressBits;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_ADDRESS_BITS, deviceAddresBitsSize, &deviceAddressBits, NULL));
            std::cout << "    Device parent: " << deviceAddressBits << std::endl;
