...
int main() {
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof global_mem_size, &global_mem_size, nullptr));
            std::cout << "        Global memory size: " << (global_mem_size / 1048576.) << "M" << std::endl;

            cl_ulong local_mem_size;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof local_mem_size, &local_mem_size, nullptr));
            std::cout << "        Local memory size: " << (local_mem_size / 1048576.) << "M" << std::endl;

            size_t max_wg_size;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof max_wg_size, &max_wg_size, nullptr));
            std::cout << "        Max work group size: " << max_wg_size << std::endl;

            cl_bool is_little_end;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_ENDIAN_LITTLE, sizeof is_little_end, &is_little_end, nullptr));
            std::cout << "        Endianess: " << (is_little_end ? "LE" : "BE") << std::endl;
        }
    }
