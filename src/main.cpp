...
int main() {
    ...
    try {
        ...
        std::cout << device_info_to_string(device) << std::endl;;
    ...
    const char* c_str_kernel_sources = kernel_sources.c_str();
    cl_program program = clCreateProgramWithSource(context, 1, &c_str_kernel_sources, nullptr, &err_code);
    OCL_SAFE_CALL(err_code);

    // TODO 8 Теперь скомпилируйте программу и напечатайте в консоль лог компиляции
    // см. clBuildProgram
    OCL_SAFE_CALL(clBuildProgram(program, 1, &device.id, nullptr, nullptr, nullptr));
    ...
    {
        unsigned int i = 0;
        clSetKernelArg(kernel, i++, sizeof(cl_mem), &a_mem_obj);
        clSetKernelArg(kernel, i++, sizeof(cl_mem), &b_mem_obj);
        clSetKernelArg(kernel, i++, sizeof(cl_mem), &c_mem_obj);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event;
            clEnqueueNDRangeKernel(queue, kernel, 1, nullptr, &global_work_size, &workGroupSize, 0, nullptr, &event);
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue, c_mem_obj, CL_TRUE, 0, n * sizeof(cl_float), cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
