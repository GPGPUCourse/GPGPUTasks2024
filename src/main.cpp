...
cl_device_id getDevice() {
    ...
    cl_device_id gpu_dev = 0, cpu_dev = 0;
    for (int plt_idx = 0; plt_idx < platformNumber; ++plt_idx) {
        cl_platform_id platform = platforms[plt_idx];
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            cl_device_type deviceTypeSize = 0, deviceType = 0;
    ...
    return gpu_dev ? gpu_dev : cpu_dev;
}
...
int main() {
    ...
    auto as_buf = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(float) * n, as.data(), &err);
    OCL_SAFE_CALL(err);
    auto bs_buf = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(float) * n, bs.data(), &err);
    OCL_SAFE_CALL(err);
    auto cs_buf = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, sizeof(float) * n, cs.data(), &err);
    OCL_SAFE_CALL(err);
...
//         std::cout << kernel_sources << std::endl;
    ...
    auto program = clCreateProgramWithSource(ctx, 1, &kernel_sources_ptr, nullptr, &err);
    OCL_SAFE_CALL(err);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            ...
            OCL_SAFE_CALL(clWaitForEvents(1, &start));
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue,
                                              cs_buf,
                                              true,
                                              0,
                                              sizeof(float) * n,
    ...
    clReleaseMemObject(cs_buf);
    clReleaseMemObject(bs_buf);
    clReleaseMemObject(as_buf);
