...
int main() {
    ...
    cl_device_id device = cpu;
    if (gpu != nullptr) {
        device = gpu;
    }

    if (device == nullptr) {
        throw std::runtime_error("Neither CPU or GPU found");
    }
    ...
    cl_mem as_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, bufferSize, as.data(),
                                   &errorCode);
    OCL_SAFE_CALL(errorCode);

    cl_mem bs_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, bufferSize, bs.data(),
                                   &errorCode);
    OCL_SAFE_CALL(errorCode);

    cl_mem cs_gpu = clCreateBuffer(context, CL_MEM_WRITE_ONLY, bufferSize, nullptr, &errorCode);
    OCL_SAFE_CALL(errorCode);
    ...
    const char *kernelSourcesData = kernel_sources.data();
    size_t kernelSourcesLength = kernel_sources.length();
    cl_program program = clCreateProgramWithSource(context, 1, &kernelSourcesData, &kernelSourcesLength, &errorCode);
    OCL_SAFE_CALL(errorCode);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            ...
            OCL_SAFE_CALL(clEnqueueNDRangeKernel(commandQueue, kernel, 1, nullptr, &global_work_size, nullptr, 0,
                                                 nullptr, &event));
    ...
    // 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event = nullptr;
            OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, cs_gpu, CL_TRUE, 0, bufferSize, cs.data(), 0, nullptr, &event));
            OCL_SAFE_CALL(clWaitForEvents(1, &event));
            OCL_SAFE_CALL(clReleaseEvent(event));
            t.nextLap();
        }
    ...
    std::cout << "CPU and GPU results are same!" << std::endl;
