...
int main(int argc, char **argv) {
    ...
    context.activate();
    {
        ...
        const unsigned int arr_size = width * height;
        ...
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(16, 16, width, height), gpu_results_buf, width, height, centralX - sizeX / 2.0f,
                        centralY - sizeY / 2.0f, sizeX, sizeY, iterationsLimit, 0, 0);
            t.nextLap();
        }

        gpu_results_buf.readN(gpu_results.ptr(), arr_size);

        size_t flopsInLoop = 10;
        size_t maxApproximateFlops = width * height * iterationsLimit * flopsInLoop;
        size_t gflops = 1000 * 1000 * 1000;
        std::cout << "GPU: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "GPU: " << static_cast<double>(maxApproximateFlops) / static_cast<double>(gflops) / t.lapAvg()
                  << " GFlops" << std::endl;
    ...
    do {
        if (!useGPU) {
            ...
            kernel.exec(gpu::WorkSize(16, 16, width, height), results_vram, width, height, centralX - sizeX / 2.0f,
                        centralY - sizeY / 2.0f, sizeX, sizeY, iterationsLimit, 0, 5 /* antialiasing level */);
