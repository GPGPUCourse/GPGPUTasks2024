                   ...
                   unsigned int iters, bool smoothing)
{
    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            ...
            // x + i *y = 0 at the start
            // but C varies and is equal to the current point of the image
            // here C = x0 + i * y0
            // 0^2 = 0, so we can start from x0 + i*y0 (just C)

            float x = x0;   // x is real part
            float y = y0;   // y is imaginary part

            // (x_prev + i * y_prev)^2 = (x_prev)^2 + 2 * (x_prev) * i * (y_prev) - (y_prev)^2
            // so x = x_prev^2 - y_prev^2 + x_0
            // so y = 2 * (x_prev) * (y_prev) + y0
            ...
            results[j * width + i] = result;    // цвет мб?
        }
    }
}
