...
void renderInWindow(float centralX, float centralY, unsigned int iterationsLimit, bool useGPU, const unsigned int i);
    ...
    const unsigned int aa = 4;
    {
        ...
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(workGroupSize, global_work_size),
                        ...
                        iterationsLimit, aa);
            gpu_results_buffer.readN(gpu_results.ptr(), n);
            t.nextLap();
        }
        size_t flopsInLoop = 10;
        size_t maxApproximateFlops = width * height * iterationsLimit * flopsInLoop * aa * aa;
        size_t gflops = 1000*1000*1000;
...
//    bool useGPU = true;
//    renderInWindow(centralX, centralY, iterationsLimit, useGPU, aa);

    return 0;
}

void renderInWindow(float centralX, float centralY, unsigned int iterationsLimit, bool useGPU, const unsigned int aa)
{
    ...
    do {
        if (!useGPU) {
            ...
            kernel.exec(gpu::WorkSize(128, width * height),
