    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            ...
            if (smoothing && iter != iters) {
                result = result - logf(logf(x * x + y * y) / logf(threshold2)) / logf(2.0f);
            }
...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        gpu::gpu_mem_32f device_gpu_results;
        device_gpu_results.resizeN(width * height);
        auto work_size = gpu::WorkSize(16, 16, width, height);

        timer t;
        for (int i = 0; i < benchmarkingIters; i++) {
            kernel.exec(work_size,
                device_gpu_results, width, height,
                centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                sizeX, sizeY,
                iterationsLimit, 0);
            t.nextLap();
        ...
        device_gpu_results.readN(gpu_results.ptr(), width * height);

        double realIterationsFraction = 0.0;
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) {
                realIterationsFraction += gpu_results.ptr()[i + j * width];
            }
        }
        std::cout << "    Real iterations fraction: " << 100.0 * realIterationsFraction / (width * height) << "%" << std::endl;
