...
int main(int argc, char **argv) {
    ...
    context.activate();
    {
        ...
        timer t;
        gpu::gpu_mem_32f resultImage;
        resultImage.resizeN(width * height);

        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(16, 16, width, height), resultImage, width, height, centralX - sizeX / 2.0f,
                        centralY - sizeY / 2.0f, sizeX, sizeY, iterationsLimit, 0);
            t.nextLap();
        }

        const auto flopsInLoop = 10;
        const auto maxApproximateFlops = width * height * iterationsLimit * flopsInLoop;
        const auto gflops = 1000 * 1000 * 1000;
        std::cout << "GPU: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "GPU: " << maxApproximateFlops / gflops / t.lapAvg() << " GFlops" << std::endl;

        resultImage.readN(gpu_results.ptr(), width * height);

        auto realIterationsFraction = 0.0;
        for (int j = 0; j < height; ++j) {
            for (int i = 0; i < width; ++i) {
