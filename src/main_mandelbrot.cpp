...
int main(int argc, char **argv) {
    ...
    context.activate();
    {
        ...
        timer t;
        auto worksize = gpu::WorkSize(16, 16, width, height);
        for (int i = 0; i < benchmarkingIters; i++) {
            kernel.exec(worksize, results_gpu, width, height, centralX - sizeX / 2.0f, centralY - sizeY / 2.0f, sizeX,
                        sizeY, iterationsLimit, 1);
            t.nextLap();
        }
